...
class StabilityAnalyzer:
    ...
    def find_stability_gaps(self, planets: list[tuple[str, float, float]]
        ...
        for i in range(len(hills) - 1):
            ...
            sep_mutual = self.mutual_hill_separation(inner, outer)

            mu = (inner.m_planet + outer.m_planet) / (3.0 * self.stellar_mass)
            a_mid = (inner.a_au + outer.a_au) / 2.0
            r_h_mutual = a_mid * mu ** (1.0 / 3.0)
            r_crit = self.critical_sep * r_h_mutual
            gap_width = (outer.a_au - inner.a_au) - r_crit

            if gap_width > 0:
                inner_boundary = inner.a_au + r_h_mutual * (self.critical_sep / 2.0)
                outer_boundary = outer.a_au - r_h_mutual * (self.critical_sep / 2.0)

                gap_ratio = (outer.a_au - inner.a_au) / max(r_h_mutual * self.critical_sep, 1e-12)
        ...
        if isinstance(system, StellarSystem):
            ...
            for body in system.planets:
                ...
                if a is not None and a > 0:
                    data.append((body.name, a, body.mass))
        ...
        if isinstance(system, ExoplanetSystem):
            ...
            for p in system.sorted_planets():
                ...
                if a is not None and a > 0:
                    ...
                    data.append((name, float(a), float(mass_solar)))
            return data

        # Assume list of (name, a, mass_solar) tuples
        return sorted(system, key=lambda x: x[1])
